...
router.get("/admin/edit/:id",isloggedIn,isOwner,async (req,res)=>{
    let user = await userModel.findOne({_id:req.user.id})
    let product = await productModel.findOne({_id:req.params.id})
    res.render("editproducts",{user,product})
})
router.post("/admin/edit/:id",isOwner,upload.single('image'),async (req,res)=>{
...
router.get("/admin/delete/:id",isOwner,async (req,res)=>{
    try{
